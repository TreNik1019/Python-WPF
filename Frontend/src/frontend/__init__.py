...
def main():
    ...
    try:
        help_result = subprocess.run([str(tailwindExe), "-h"], capture_output=True, text=True, check=True)
        ...
        if local_version_match:
            ...
            req = urllib.request.Request("https://github.com/tailwindlabs/tailwindcss/releases/latest", method="HEAD")
            with urllib.request.urlopen(req, timeout=3) as response:
                ...
                if latest_version_match:
                    ...
                    if local_version != latest_version:
                        print()
                        print("------------------------------------------------------------------------")
                        print(f"\033[93mHINWEIS: Deine lokale Tailwind-Version ({local_version}) ist veraltet!\033[0m")
